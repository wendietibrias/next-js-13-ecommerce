    ...
    return (
        <SkeletonTheme baseColor="#ecf0f1" highlightColor="#fff">
            ...
            <div className="w-[450px] lg:w-full">
              ...
              {loading ? (
                 <Skeleton count={4} width="23%" height="80px" containerClassName="flex flex-wrap mt-5 justify-between items-center"/>
              ) : (
            ...
            <div className="flex-1 lg:w-full">
                <div className="flex items-center gap-x-7">
                    {loading ? <Skeleton width={windowSize < 620 && windowSize != 0 ? "210px" : "400px"} height="30px" /> : <h2 className="text-xl font-bold text-gray-700 sm:text-sm">{product?.title}</h2>}
                    {loading ? <Skeleton width="80px"  height="30px"  /> : <h5 className="text-md font-bold text-blue-500">{convertMoney(product?.price || 0)}</h5>}
                </div>
                {loading ? (
                    <div className="mt-3">
                   ...
                   <div className="w-full">
                        <p className="text-sm sm:text-[13px] text-gray-400 mt-1">{product?.category?.title}</p>
                        <p className="text-sm sm:text-[13px] text-gray-400 mt-5">{product?.excerpt}</p>
                        <p className="text-sm sm:text-[13px] text-gray-400 mt-1">{product?.stock} Stocks</p>
                        <p className="text-sm sm:text-[13px] mt-2 text-gray-400">{product?.description}</p>
                   </div>
                )}
                <div className="mt-7">
                    <div className="flex items-center gap-x-3">
                       {loading ? <Skeleton width="30px" height="15px"/> :  <button onClick={()=>setQty(qty <= 1 ? 1 : qty - 1)} className="text-blue-500 font-semibold text-lg">-</button>}
                        {loading ? <Skeleton width="20px" height="16px" /> : <span className="text-gray-700 text-[13px] font-bold">{qty}</span>}
                        {loading ? <Skeleton width="30px" height="15px" /> : <button onClick={() => setQty(qty+1)} className="text-blue-500 font-semibold text-lg">+</button>}
                    </div>
                    <div className="flex items-center gap-x-3">
                     {loading ? (
