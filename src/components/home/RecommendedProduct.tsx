...
const RecommendedProduct = () => {
  ...
  const [sizeWindow, setSizeWindow] = useState<number>(
    typeof window !== "undefined" ? window.innerWidth : 1920
  );

  const getRecommendedProduct = async () => {
    try {
      const fetchRecommendedProduct =
        await sanityClient.fetch(`*[_type == "product" && recommended == true] {
             title,
             price,
             thumbnail,
             slug,
  ...
  return (
    <SkeletonTheme baseColor="#ecf0f1" highlightColor="#fff">
      <div className="mt-7 lg:px-3 sm:mt-10">
        <h3 className="text-lg sm:text-sm font-bold text-gray-700">
          Recommended Product
        </h3>
        <div className="grid grid-cols-4 lg:grid-cols-2 xs:grid-cols-1 gap-4 mt-5 sm:mt-3">
          {loading
            ? arrayDummy.map((item: number) => (
                <SkeletonProductCard key={item} />
              ))
            : products.map((product: IProductPromise, idx: number) => (
                <ProductCard key={idx} product={product} loading={loading} />
              ))}
        </div>
      </div>
    </SkeletonTheme>
