    ...
    <SkeletonTheme baseColor="#ecf0f1" highlightColor="#fff">
      <div className="w-full mt-10 lg:px-3">
        <div className="flex justify-between items-center">
          ...
          <Link href="/product">
            <button className="text-blue-500 text-[12px] font-semibold">
              See more
            </button>
          </Link>
        </div>
        <div className="grid grid-cols-4 sm:grid-cols-2 lg:grid-cols-3 xs:grid-cols-1 sm:gap-2 gap-4 mt-5 sm:mt-3">
          {loading
            ? arrayDummy.map((item: number) => (
                <SkeletonProductCard key={item} />
              ))
            : products.map((product: IProductPromise, idx: number) => (
                <ProductCard key={idx} loading={loading} product={product} />
              ))}
        </div>
      </div>
    </SkeletonTheme>
