          ...
          <Slider {...settings}>
           {bannerItems.map((banner : IBannerPromise , idx : number) => (
             <div className={`w-full relative overflow-hidden`} key={idx}>
               <img src={urlFor(banner?.thumbnail?.asset?._ref).url()} alt={banner?.title} className="w-full object-cover rounded-lg h-[400px]"/>
               <div className="w-full absolute top-0 left-0 rounded-lg bg-gray-700/50 flex h-full items-center">
                 <div className="w-full text-white px-10">
                   <h2 className="text-3xl font-extrabold capitalize">{banner?.title}</h2>
                   <p className="text-sm text-white font-medium mt-3">{banner?.description}</p>
                   <button className="bg-white p-3 mt-7 rounded-md text-gray-700 text-[13px] font-semibold">Explore Now</button>
                 </div>
               </div>
             </div>
           ))}
