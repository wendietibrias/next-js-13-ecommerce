   ...
   if(status === "authenticated") {
    ...
    return (
        <form onSubmit={handleSubmit(submitHandler)} className="w-full mt-7 flex flex-col gap-y-3">
            <Input showlable={true} register={register} name="email" placeholder="example@gmail.com" error={errors?.email ? true : false} type="email" />
            <Input showlable={true} register={register} name="password" placeholder="Password" error={errors?.password ? true : false} type="password" />
